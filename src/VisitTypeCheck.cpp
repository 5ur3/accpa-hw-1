...
public:
  ...
  StellaType(){};
  StellaType(std::string type_string) { this->type_string = type_string; };
...
public:
  ...
  virtual void proxyExpressionTypeToken(std::string typeToken) = 0;
  virtual void proxyExpression(StellaExpression *expression) = 0;
  virtual void proxyIdent(Stella::StellaIdent ident) = 0;
};

class StellaConstIntExpression : public StellaExpression {
public:
  StellaConstIntExpression() { this->type = STELLA_EXPRESIION_TYPE_CONST_INT; }
  StellaType getStellaType() { return StellaType("nat"); }
  bool isTypingCorrect() { return true; }
...
public:
  ...
  bool isTypingCorrect() { return this->expression->isTypingCorrect(); }
  void proxyExpressionTypeToken(std::string typeToken) {
...
public:
  ...
  StellaFunction() {}
  StellaFunction(Stella::StellaIdent ident) { this->ident = ident; }
